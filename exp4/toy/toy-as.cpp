...
static void WriteOutputFile(const Module *M,std::string name) {
  ...
  */
  ...
  std::unique_ptr<ToolOutputFile> Out(
      new ToolOutputFile(name, EC, sys::fs::F_None));
  if (EC) {
    errs() << EC.message() << '\n';
    exit(1);
  }
